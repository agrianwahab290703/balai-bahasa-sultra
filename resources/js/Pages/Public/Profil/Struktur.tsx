...
import { Network, ChevronRight, ArrowLeft, X, ZoomIn, FileText } from 'lucide-react';
import { AnimatePresence, motion } from 'framer-motion';

type StrukturSection = {
    id: number;
    type: string;
    title: string;
    content: string;
...
interface StrukturProps {
    sections?: StrukturSection[];
}

export default function Struktur({ sections = [] }: StrukturProps) {
    ...
    const getImageUrls = (section: StrukturSection): string[] => {
        if (Array.isArray(section.image_urls) && section.image_urls.length > 0) {
            return section.image_urls.filter((url): url is string => Boolean(url));
        }

        if (Array.isArray(section.images) && section.images.length > 0) {
            return section.images.filter((url): url is string => Boolean(url));
        }
            ...
            <PublicLayout title="Struktur Organisasi" description="Struktur organisasi Kantor Bahasa Sulawesi Tenggara">
                <section className="min-h-[70vh] flex items-center justify-center bg-white">
                    <div className="text-center">
                        <p className="text-sm uppercase tracking-[0.4em] text-gray-300">Struktur Organisasi</p>
                        <h1 className="text-3xl font-semibold text-gray-400 mt-4">Belum Ada Konten</h1>
                    </div>
                </section>
            </PublicLayout>
        );
                ...
                <div className="container relative mx-auto px-4">
                    <div className="max-w-4xl mx-auto text-center">
                        ...
                        <p className="text-lg text-blue-100/90 max-w-2xl mx-auto leading-relaxed">
                            Susunan organisasi Kantor Bahasa Provinsi Sulawesi Tenggara. Konten akan diperbarui secara otomatis berdasarkan data yang Anda simpan melalui panel admin.
                        </p>
                    </div>
                </div>
                    ...
                    {sortedSections.length > 0 ? (
                        ...
                        >
                            ...
                            <p className="text-gray-500 leading-relaxed">
                                Struktur organisasi akan tampil otomatis setelah Anda menambahkan konten melalui halaman admin.
                            </p>
                        </motion.div>
                    )}
                </div>
                ...
                <div className="container mx-auto px-4">
                    <Card className="border-0 bg-gradient-to-r from-blue-600 to-blue-700 shadow-xl max-w-4xl mx-auto">
                        <CardContent className="p-8 sm:p-12 text-center text-white">
                            <h2 className="text-2xl font-bold sm:text-3xl mb-4">Butuh Informasi Tambahan?</h2>
                            <p className="text-blue-100 mb-6 max-w-2xl mx-auto">
                                Hubungi kami jika Anda membutuhkan penjelasan lebih lanjut mengenai struktur organisasi atau layanan Balai Bahasa Provinsi Sulawesi Tenggara.
                            </p>
