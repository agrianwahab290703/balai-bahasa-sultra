...
import { 
    ChevronRight, ArrowLeft, ClipboardCheck, FileText, FolderOpen, CheckCircle2, Shield, X, Loader2, ExternalLink, ChevronDown, FileCheck, Target, BarChart3, CheckSquare, Clock, FileSpreadsheet, Monitor, Users
} from 'lucide-react';
...
const documentIcons = [FileCheck, Target, BarChart3, CheckSquare, Clock, FileSpreadsheet, Monitor, Users];
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-white">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">Dokumen Pengelolaan Akuntabilitas</p>
                        </div>
                    </div>
...
const DocumentCard = ({ document, index, onViewDocuments }: { document: DocumentItem; index: number; onViewDocuments: (doc: DocumentItem) => void; }) => {
    ...
    const colorSchemes = [
        { bg: 'from-cyan-500 to-cyan-600', shadow: 'shadow-cyan-500/25', light: 'bg-cyan-50', text: 'text-cyan-600' },
        ...
        { bg: 'from-sky-500 to-sky-600', shadow: 'shadow-sky-500/25', light: 'bg-sky-50', text: 'text-sky-600' },
                ...
                <div className="p-6 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <h3 className="text-base lg:text-lg font-semibold text-gray-900 leading-relaxed group-hover:text-cyan-600 transition-colors">{document.title}</h3>
...
export default function PengelolaanAkuntabilitas({ documents }: Props) {
    const [selectedDocument, setSelectedDocument] = useState<DocumentItem | null>(null);

    return (
        <PublicLayout title="Pengelolaan Akuntabilitas Kinerja - ZI-WBK" description="Pengelolaan Akuntabilitas Kinerja">
            <section className="relative bg-gradient-to-br from-[#0891b2] via-[#0e7490] to-[#155e75] py-16 lg:py-20 overflow-hidden">
                ...
                <div className="container relative mx-auto px-4">
                    <div className="max-w-4xl mx-auto">
                        <div className="flex items-center gap-2 text-cyan-200 mb-6 text-sm flex-wrap">
                            <Link href="/" className="hover:text-white transition-colors">Beranda</Link>
                            <ChevronRight className="h-4 w-4" />
                            <Link href="/zi-wbk" className="hover:text-white transition-colors">ZI-WBK</Link>
                            <ChevronRight className="h-4 w-4" />
                            <Link href="/zi-wbk/akuntabilitas-kerja" className="hover:text-white transition-colors">Akuntabilitas Kerja</Link>
                            <ChevronRight className="h-4 w-4" />
                            <span className="text-white">Pengelolaan Akuntabilitas</span>
                        </div>
                        <div className="flex items-start gap-6">
                            ...
                            <div>
                                <Badge className="mb-4 bg-yellow-400/90 text-gray-900 hover:bg-yellow-400 font-medium px-4 py-1.5">
                                    <Shield className="w-4 h-4 mr-2" />Akuntabilitas Kerja
                                </Badge>
                                <h1 className="text-3xl font-bold text-white sm:text-4xl lg:text-5xl mb-4">
                                    Pengelolaan Akuntabilitas <span className="font-display italic text-yellow-400">Kinerja</span>
                                </h1>
                                <p className="text-lg text-cyan-100/90 max-w-2xl leading-relaxed">
                                    Pengelolaan akuntabilitas kinerja melalui perencanaan, penetapan IKU, dan pelaporan kinerja.
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            <Card className="border-0 shadow-lg bg-gradient-to-br from-cyan-50 to-white overflow-hidden">
                                <CardContent className="p-6 lg:p-8">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div>
                                            <h2 className="text-xl font-bold text-gray-900 mb-2">Dokumen Pengelolaan Akuntabilitas</h2>
                                            <p className="text-gray-600 leading-relaxed">
                                                Dokumen terkait perencanaan, penetapan IKU, dan pelaporan kinerja unit kerja.
                                            </p>
                                        </div>
                                    </div>
                                </CardContent>
                            ...
                            <Card className="border-0 shadow-lg bg-gradient-to-br from-gray-50 to-white">
                                <CardContent className="p-6">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div>
                                            <h3 className="font-semibold text-gray-900 mb-1">Akuntabilitas Kinerja</h3>
                                            <p className="text-sm text-gray-600 leading-relaxed">
                                                Pengelolaan akuntabilitas kinerja yang baik memastikan tercapainya target dan transparansi dalam pelaksanaan tugas.
                                            </p>
                                        </div>
                                    </div>
                                </CardContent>
                ...
                <div className="container mx-auto px-4">
                    <div className="max-w-4xl mx-auto">
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                            <Link href="/zi-wbk/akuntabilitas-kerja/keterlibatan-pimpinan"><Button variant="outline" className="gap-2"><ArrowLeft className="w-4 h-4" />Keterlibatan Pimpinan</Button></Link>
                            <Link href="/zi-wbk"><Button className="gap-2 bg-cyan-600 hover:bg-cyan-700">Kembali ke ZI-WBK<ChevronRight className="w-4 h-4" /></Button></Link>
                        </div>
                    </div>
                </div>
            </section>
