...
import { 
    ChevronRight, ArrowLeft, ShieldCheck, FileText, FolderOpen, CheckCircle2, Shield, X, Loader2, ExternalLink, ChevronDown, Building, AlertTriangle, Settings, Megaphone, FileDown
} from 'lucide-react';
import { cn } from '@/lib/utils';

interface DocumentItem { id: number; title: string; description: string; folderId: string; pdfUrl?: string; }
interface Props { documents: DocumentItem[]; }

const documentIcons = [Building, AlertTriangle, Settings, Megaphone, FileDown];

const DocumentViewer = ({ folderId, title, onClose }: { folderId: string; title: string; onClose: () => void; }) => {
    const [isLoading, setIsLoading] = useState(true);
    const isPlaceholder = folderId.includes('PLACEHOLDER') || folderId === 'PDF_LINK';
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-white">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">Dokumen SPIP</p>
                        </div>
                    </div>
...
const DocumentCard = ({ document, index, onViewDocuments }: { document: DocumentItem; index: number; onViewDocuments: (doc: DocumentItem) => void; }) => {
    ...
    const isPdfLink = document.folderId === 'PDF_LINK' && document.pdfUrl;
    const colorSchemes = [
        { bg: 'from-blue-500 to-blue-600', shadow: 'shadow-blue-500/25', light: 'bg-blue-50', text: 'text-blue-600' },
        { bg: 'from-indigo-500 to-indigo-600', shadow: 'shadow-indigo-500/25', light: 'bg-indigo-50', text: 'text-indigo-600' },
        { bg: 'from-violet-500 to-violet-600', shadow: 'shadow-violet-500/25', light: 'bg-violet-50', text: 'text-violet-600' },
        { bg: 'from-purple-500 to-purple-600', shadow: 'shadow-purple-500/25', light: 'bg-purple-50', text: 'text-purple-600' },
            ...
            <CardContent className="p-0">
                <div className="p-6 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <h3 className="text-base lg:text-lg font-semibold text-gray-900 leading-relaxed group-hover:text-blue-600 transition-colors">{document.title}</h3>
                ...
                <div className={cn("overflow-hidden transition-all duration-300", isExpanded ? "max-h-32 opacity-100" : "max-h-0 opacity-0")}>
                    <div className="px-6 pb-6 pt-2 border-t border-gray-100 bg-gradient-to-b from-gray-50/50 to-white">
                        <div className="flex items-center gap-3">
                            {isPdfLink ? (
                                <Button onClick={(e) => { e.stopPropagation(); window.open(document.pdfUrl, '_blank'); }} className={cn("flex-1 text-white shadow-lg", `bg-gradient-to-r ${colors.bg}`, colors.shadow)}>
...
export default function PenerapanSpip({ documents }: Props) {
    const [selectedDocument, setSelectedDocument] = useState<DocumentItem | null>(null);

    return (
        <PublicLayout title="Penerapan SPIP - ZI-WBK" description="Penerapan Sistem Pengendalian Intern Pemerintah">
            <section className="relative bg-gradient-to-br from-[#2563eb] via-[#1d4ed8] to-[#1e40af] py-16 lg:py-20 overflow-hidden">
                <div className="absolute inset-0 overflow-hidden">
                    <div className="absolute -top-24 -right-24 w-96 h-96 bg-yellow-400/10 rounded-full blur-3xl" />
                    <div className="absolute -bottom-32 -left-32 w-[500px] h-[500px] bg-blue-300/20 rounded-full blur-3xl" />
                </div>
                ...
                <div className="container relative mx-auto px-4">
                    <div className="max-w-4xl mx-auto">
                        <div className="flex items-center gap-2 text-blue-200 mb-6 text-sm flex-wrap">
                            ...
                            <ChevronRight className="h-4 w-4" />
                            <span className="text-white">Penerapan SPIP</span>
                        </div>
                        <div className="flex items-start gap-6">
                            ...
                            <div>
                                ...
                                <h1 className="text-3xl font-bold text-white sm:text-4xl lg:text-5xl mb-4">
                                    Penerapan <span className="font-display italic text-yellow-400">SPIP</span>
                                </h1>
                                <p className="text-lg text-blue-100/90 max-w-2xl leading-relaxed">
                                    Sistem Pengendalian Intern Pemerintah untuk meminimalisir risiko.
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            <Card className="border-0 shadow-lg bg-gradient-to-br from-blue-50 to-white overflow-hidden">
                                <CardContent className="p-6 lg:p-8">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div>
                                            <h2 className="text-xl font-bold text-gray-900 mb-2">Dokumen SPIP</h2>
                                            <p className="text-gray-600 leading-relaxed">
                                                Dokumen penerapan Sistem Pengendalian Intern Pemerintah.
                                            </p>
                                        </div>
                                    </div>
                                </CardContent>
                            ...
                            <Card className="border-0 shadow-lg bg-gradient-to-br from-gray-50 to-white">
                                <CardContent className="p-6">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div>
                                            <h3 className="font-semibold text-gray-900 mb-1">Pengendalian Internal</h3>
                                            <p className="text-sm text-gray-600 leading-relaxed">
                                                SPIP memastikan efektivitas dan efisiensi pencapaian tujuan, keandalan pelaporan, dan kepatuhan pada peraturan.
                                            </p>
                                        </div>
                                    </div>
                                </CardContent>
                ...
                <div className="container mx-auto px-4">
                    <div className="max-w-4xl mx-auto">
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                            <Link href="/zi-wbk/penguatan-pengawasan/pengendalian-gratifikasi"><Button variant="outline" className="gap-2"><ArrowLeft className="w-4 h-4" />Pengendalian Gratifikasi</Button></Link>
                            <Link href="/zi-wbk/penguatan-pengawasan/pengaduan-masyarakat"><Button className="gap-2 bg-blue-600 hover:bg-blue-700">Pengaduan Masyarakat<ChevronRight className="w-4 h-4" /></Button></Link>
                        </div>
                    </div>
                </div>
            </section>
