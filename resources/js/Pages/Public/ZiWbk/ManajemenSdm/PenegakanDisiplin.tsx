...
import { 
    ChevronRight, ArrowLeft, Scale, FileText, FolderOpen, CheckCircle2, Shield, X, Loader2, ExternalLink, ChevronDown, Gavel
} from 'lucide-react';
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-white">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">Dokumen Penegakan Disiplin</p>
                        </div>
                    </div>
...
const DocumentCard = ({ document, index, onViewDocuments }: { document: DocumentItem; index: number; onViewDocuments: (doc: DocumentItem) => void; }) => {
    ...
    const colors = { bg: 'from-slate-600 to-slate-700', shadow: 'shadow-slate-600/25', light: 'bg-slate-100', text: 'text-slate-700' };
                ...
                <div className="p-6 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <h3 className="text-base lg:text-lg font-semibold text-gray-900 leading-relaxed group-hover:text-slate-700 transition-colors">{document.title}</h3>
...
export default function PenegakanDisiplin({ documents }: Props) {
    const [selectedDocument, setSelectedDocument] = useState<DocumentItem | null>(null);

    return (
        <PublicLayout title="Penegakan Aturan Disiplin - ZI-WBK" description="Penegakan Aturan Disiplin/Kode Etik/Kode Perilaku Pegawai">
            <section className="relative bg-gradient-to-br from-[#475569] via-[#334155] to-[#1e293b] py-16 lg:py-20 overflow-hidden">
                <div className="absolute inset-0 overflow-hidden">
                    <div className="absolute -top-24 -right-24 w-96 h-96 bg-yellow-400/10 rounded-full blur-3xl" />
                    <div className="absolute -bottom-32 -left-32 w-[500px] h-[500px] bg-slate-400/20 rounded-full blur-3xl" />
                </div>
                ...
                <div className="container relative mx-auto px-4">
                    <div className="max-w-4xl mx-auto">
                        <div className="flex items-center gap-2 text-slate-300 mb-6 text-sm flex-wrap">
                            ...
                            <ChevronRight className="h-4 w-4" />
                            <span className="text-white">Penegakan Disiplin</span>
                        </div>
                        <div className="flex items-start gap-6">
                            ...
                            <div>
                                ...
                                <h1 className="text-3xl font-bold text-white sm:text-4xl lg:text-5xl mb-4">
                                    Penegakan Aturan <span className="font-display italic text-yellow-400">Disiplin</span>
                                </h1>
                                <p className="text-lg text-slate-200/90 max-w-2xl leading-relaxed">
                                    Implementasi aturan disiplin, kode etik, dan kode perilaku pegawai.
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="mb-10">
                            <Card className="border-0 shadow-lg bg-gradient-to-br from-slate-50 to-white overflow-hidden">
                                <CardContent className="p-6 lg:p-8">
                                    <div className="flex items-start gap-4">
                                        <div className="shrink-0 w-12 h-12 bg-slate-700 rounded-xl flex items-center justify-center">
                                            <Scale className="w-6 h-6 text-white" />
                                        </div>
                                        <div>
                                            <h2 className="text-xl font-bold text-gray-900 mb-2">Dokumen Penegakan Disiplin</h2>
                                            <p className="text-gray-600 leading-relaxed">
                                                Dokumen implementasi aturan disiplin, kode etik, dan kode perilaku pegawai.
                                            </p>
                                        </div>
                                    </div>
                                </CardContent>
                        ...
                        <div className="mt-8">
                            <Card className="border-0 shadow-lg bg-gradient-to-br from-gray-50 to-white">
                                <CardContent className="p-6">
                                    <div className="flex items-start gap-4">
                                        <CheckCircle2 className="w-6 h-6 text-slate-700 shrink-0 mt-0.5" />
                                        <div>
                                            <h3 className="font-semibold text-gray-900 mb-1">Integritas Pegawai</h3>
                                            <p className="text-sm text-gray-600 leading-relaxed">
                                                Penegakan disiplin dan kode etik merupakan fondasi dalam membangun integritas dan profesionalisme pegawai.
                                            </p>
                                        </div>
                                    </div>
                                </CardContent>
                ...
                <div className="container mx-auto px-4">
                    <div className="max-w-4xl mx-auto">
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                            <Link href="/zi-wbk/manajemen-sdm/sistem-informasi"><Button variant="outline" className="gap-2"><ArrowLeft className="w-4 h-4" />Sistem Informasi</Button></Link>
                            <Link href="/zi-wbk"><Button className="gap-2 bg-slate-700 hover:bg-slate-800">Kembali ke ZI-WBK<ChevronRight className="w-4 h-4" /></Button></Link>
                        </div>
                    </div>
                </div>
            </section>
