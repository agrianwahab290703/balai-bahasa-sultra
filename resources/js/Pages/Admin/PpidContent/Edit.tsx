...
export default function Edit({ content, categories, subCategories }: Props) {
  ...
  const [hasExistingDocument, setHasExistingDocument] = useState(!!content.document_path);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('admin.ppid-content.update', content.id), {
      forceFormData: true,
    });
  };
  ...
  const getDocumentName = () => {
    ...
    if (hasExistingDocument && content.document_path) {
      return content.document_path.split('/').pop() || 'Dokumen';
    }
    return null;
  };
  ...
  return (
    <AdminLayout>
      ...
      <div className="space-y-6">
        {/* Hero Header */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-500 via-orange-500 to-red-500 p-6 lg:p-8">
          ...
          <div className="relative z-10">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl lg:text-3xl font-bold text-white">Edit Konten PPID</h1>
                  <p className="text-orange-100 mt-1 truncate max-w-md">{content.title}</p>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div>
                    ...
                    />
                    <p className="text-xs text-gray-400 mt-1">Slug akan diperbarui otomatis saat judul berubah</p>
                  </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    />
                    {imagePreview ? (
                      <div className="relative border-2 border-amber-200 bg-amber-50 rounded-xl p-4">
                        <img
                          src={imagePreview}
                          alt="Preview"
                        ...
                        <div className="absolute top-2 right-2 flex gap-2">
                          <label
                            htmlFor="image"
                            className="inline-flex items-center px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 cursor-pointer"
                          >
                            Ganti
                          </label>
                          <Button
                      ...
                      <label
                        htmlFor="image"
                        className="block border-2 border-dashed border-gray-300 rounded-xl p-8 text-center cursor-pointer hover:border-amber-400 hover:bg-amber-50/50 transition-all group"
                      >
                        <div className="w-12 h-12 rounded-xl bg-gray-100 group-hover:bg-amber-100 flex items-center justify-center mx-auto mb-3 transition-colors">
                          <ImageIcon className="h-6 w-6 text-gray-400 group-hover:text-amber-500" />
                        </div>
                  ...
                  <div>
                    ...
                    />
                    {(data.document || hasExistingDocument) ? (
                      <div className="border-2 border-blue-200 bg-blue-50 rounded-xl p-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-gray-900 truncate">{getDocumentName()}</p>
                            {data.document && (
                              <p className="text-sm text-gray-500">
                                {(data.document.size / 1024 / 1024).toFixed(2)} MB
                              </p>
                            )}
                            {hasExistingDocument && !data.document && (
                              <p className="text-sm text-gray-500">Dokumen tersimpan</p>
                            )}
                          </div>
            ...
            <div className="lg:col-span-1">
              <div className="space-y-6 lg:sticky lg:top-6">
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    <div className="text-sm text-gray-500 space-y-1 pt-2 border-t border-gray-100">
                      <p>Dibuat: {new Date(content.created_at).toLocaleDateString('id-ID')}</p>
                      {content.published_at && (
                        <p>Dipublikasi: {new Date(content.published_at).toLocaleDateString('id-ID')}</p>
                      )}
                    </div>
                    ...
                    <div className="pt-3 space-y-2">
                      <Button
                        type="submit"
                        disabled={processing}
                        className="w-full h-11 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-sm font-medium shadow-lg shadow-amber-500/25"
                      >
                        {processing ? (
                          <>
                ...
                <div className="bg-red-50 rounded-xl border border-red-100 p-5">
                  <h4 className="font-medium text-red-900 mb-3">Zona Berbahaya</h4>
                  <Button
                    type="button"
                    variant="destructive"
                    className="w-full"
                    onClick={() => {
                      if (confirm('Apakah Anda yakin ingin menghapus konten ini? Tindakan ini tidak dapat dibatalkan.')) {
                        window.location.href = route('admin.ppid-content.index');
                      }
                    }}
                  >
