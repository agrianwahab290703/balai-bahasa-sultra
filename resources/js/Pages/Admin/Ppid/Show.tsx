...
import { ArrowLeft, Pencil, Trash2, Download, FileText, Calendar, Eye } from 'lucide-react';
...
export default function Show({ document, categories }: Props) {
  const handleDelete = () => {
    if (confirm('Hapus dokumen ini? Aksi ini tidak dapat dibatalkan.')) {
      router.delete(route('admin.ppid.destroy', document.id));
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" asChild>
              <Link href={route('admin.ppid.index')}>
                <ArrowLeft className="h-4 w-4" />
              </Link>
            </Button>
            <div>
              <h1 className="text-2xl font-semibold">{document.title}</h1>
              <p className="text-muted-foreground">Detail dokumen PPID</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" asChild>
              <Link href={route('admin.ppid.edit', document.id)}>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                  ...
                  {document.file_url && (
                    <Button asChild>
                      <a href={route('admin.ppid.download', document.id)} target="_blank">
